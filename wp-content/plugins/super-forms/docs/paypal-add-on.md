...
[Download link](https://codecanyon.net/item/super-forms-paypal-addon/21048964)
...
* Update post status after payment completed (only for [Front-end Posting Add-on](front-end-posting-add-on.md))
* Registered user login status after payment complete (only for [Register &amp; Login Add-on](register-login-add-on.md))
* Change user role after payment complete (only for [Register &amp; Login Add-on](register-login-add-on.md))
...
* [Calculator Add-on](calculator-add-on.md)
* [Front-end Posting Add-on](front-end-posting-add-on.md)
* [Register &amp; Login Add-on](register-login-add-on.md)
...
After you have installed the add-on successfully you can go to `Super Forms > Create Form` (or edit an existing form of your choosing).
...
In order to be able to checkout with PayPal, PayPal must know what type of payment it is that you are doing. The add-on allows you to handle 4 different payment methods.
...
Let's say you have a [dynamic column](columns.md#dynamic-add-more) setup with super forms with inside quantity element, dropdown for product name, and a variable field that is updated based on the dropdown option.
 ...
 You will be able to simply enter the following [{tags}](tags-system.md) and Super Forms - PayPal add-on will automatically add all the available options to the PayPal checkout.
